...
class model(Model):
    def __init__(self, n_out = 3):
        ...
        self.att1 = GATConv(hidden_states, attn_heads=10, dropout_rate=0.4, activation = "relu") #required keywords is channels/hidden states
        self.att2 = GATConv(hidden_states//2, attn_heads=5, dropout_rate=0.1, activation = "relu")# attn heads are the time limiting key_word, watch out with it
        self.att3 = GATConv(hidden_states*2, attn_heads=20, dropout_rate=0.7, activation = "relu")  # hiddenstates has to be pretty low as well
