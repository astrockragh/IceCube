...
def train_model(construct_dict):
    ...
    for j in range(epochs):
        for i in range(n_steps):
            if j>=10:
                i=np.random.randint(0,10, size=1)
            dataset_train=graph_data(**construct_dict['data_params'], traintest='train', i_train=int(i))
            loader_train = DisjointLoader(dataset_train, epochs=1, batch_size=batch_size)
            for batch in loader_train:
                ...
                # if current_batch == :
                    ...
                    dataset_val=graph_data(**construct_dict['data_params'], traintest='test', i_test=int(k))
