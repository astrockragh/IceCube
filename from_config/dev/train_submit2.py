...
def train_model(construct_dict):
    ...
    steps_per_epoch=loader_train.steps_per_epoch+loader_test.steps_per_epoch
    tot_time=0
    current_batch = 0
    current_epoch = 1
    loss          = 0
    ...
    for j in range(epochs):
        for k in ['test', 'train']:
            for i in range(n_steps):
                dataset_train=graph_data(**construct_dict['data_params'], traintest=k, i_train=i, i_test=i)
                loader_train = DisjointLoader(dataset_train, epochs=1, batch_size=batch_size)
